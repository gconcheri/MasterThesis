#!/bin/bash
#SBATCH --job-name=PhaseDiagram
#SBATCH --chdir=./
#SBATCH --output=./PhaseDiagram.%A_%3a.out  # %J=jobid.step, %N=node.
#
...
echo "Running task $SLURM_ARRAY_TASK_ID specified in PhaseDiagram.config.pkl on $HOSTNAME at $(date) with $USE_NUM_THREADS threads"
python /home/t30/pol/go56vod/Desktop/MasterThesis/cluster_jobs.py run PhaseDiagram.config.pkl $SLURM_ARRAY_TASK_ID
# if you want to redirect output to file, you can append the following to the line above:
#     &> "PhaseDiagram.task_$SLURM_ARRAY_TASK_ID.out"
